...
Landing page: dpnation.github.io/marketing/pmm-skills
---
Skills included
...
Recommended order:
Run `product-marketing-context` first to create the shared context file
Use any other skill — it reads the context file and skips basic questions
---
Installation (Enterprise admins)
Download the `.skill` files from the `/pmm-skills` directory
In Claude Enterprise Admin Console → Skills → Install shared skill
Upload each `.skill` file — they'll be available to all users on the instance
---
Related tools
Battlecard Builder — generates formatted `.docx` battlecards using competitive intel; pairs directly with the `competitive-intelligence` skill
Competitive Intel Site — weekly automated scan of 10+ competitors; feeds the Battlecard Builder
---
Repo structure
```
marketing/
